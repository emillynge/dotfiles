...
function renderAccounts() {	
	...
	$("body").css({"background-color":localStorage.widgetBackgroundColor, "background-image":"-webkit-gradient(linear, left top, left bottom, from(rgba(0, 0, 0, 0)), to(rgba(255,255,255,0.5)))"});
  	/* background gradient "from" is tranparent so takes the background-color, the "to" goes to white slighlty opaque */
  	//background-image: -webkit-gradient(linear, left top, left bottom, from(rgba(0, 0, 0, 0)), to(rgba(255,255,255,0.5)));
	
	previousWidgetAccounts = localStorage.widgetAccounts;
	accounts = JSON.parse(localStorage.widgetAccounts);
	...
	if (localStorage.widgetShowEmailAddresses == "false") {
		$(".accountEmail").hide();
	}
	if (localStorage.widgetShowEmailPreview == "false") {
		$(".emails").hide();
	}
	if (localStorage.widgetShowEmailSummary == "false") {
		$(".summary").hide();
	}
