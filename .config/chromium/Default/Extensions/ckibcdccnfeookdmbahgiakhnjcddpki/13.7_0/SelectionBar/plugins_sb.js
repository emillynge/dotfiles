var plugins_sb = [{
...

 {
		...
		onclick:function (scope) {
			debugger
			var asd=[
				{name:'Facebook',key:'facebook',url:'http://www.facebook.com/sharer/sharer.php?s=100&p[summary]=Taken with Webpage Screenshot&p[title]={page_title}&p[url]={page_url}#{text}'}
				,{name:'Twitter',key:'twitter',url:'http://twitter.com/home?status=%s'}
				,{name:'Google+',key:'gplus',url:'https://plus.google.com/share?url={page_url}#{text}'}
				,{name:'Evernote',key:'evernote',url:'http://s.evernote.com/grclip?url={page_url}&title={text}'}
				,{name:'Gmail',key:'gmail',url:'https://mail.google.com/mail/?view=cm&fs=1&su={page_title}&body={text} {page_url}'}
				,{name:'del.icio.us',key:'del',url:'http://del.icio.us/post?url={page_url}&title={text}'}
				,{name:'Digg',key:'digg',url:'http://digg.com/submit?url={page_url}&title={text}'}
				]
			var $toolbar=$('<div></div>').css({position:'absolute',top:scope.event.pageY,left:scope.event.pageX}).appendTo(document.body)
...

 {
		...
		onclick: function() {
			...
			jQuery('.msg',$toolbar).each(function () {jQuery(this).html(chrome.i18n.getMessage(jQuery(this).attr('tag'))); });
			...
			$('button.save',$toolbar).on('click',function (){
				$('[plugin-key=save]').trigger($.Event({type:'click'}))
			})
			...
			$('button.share',$toolbar).on('click',function (){
				$('[plugin-key=webpagescreenshot]').trigger($.Event({type:'click'}))
			})
...

{
		...
		onclick: function(scope) {
			var translateText=function(text,target,callback){
					$.ajax({showProgress:false,url:'https://www.googleapis.com/language/translate/v2?key=AIzaSyCUXax8SFsdoWpTOwWL6xW6nk2wVYs-aNo&target=' +  target + '&q=' + text,
					success:function (e){
			...
			var createSelectElement=function(inLang){
				...
				for(var x in inLang){
					if (currentLang==inLang[x].code) isSelected=' selected'; else isSelected='';
			...
			var init=function () {
				// chrome.i18n.getAcceptLanguages(function(lang) {
					...
					{lang:'Afrikaans',code:'af'},{lang:'Albanian',code:'sq'},{lang:'Arabic',code:'ar'},{lang:'Azerbaijani',code:'az'},{lang:'Basque',code:'eu'},{lang:'Bengali',code:'bn'},{lang:'Belarusian',code:'be'},{lang:'Bulgarian',code:'bg'},{lang:'Catalan',code:'ca'},{lang:'Chinese Simplified',code:'zh-CN'	},{lang:'Chinese Traditional',code:'zh-TW'	},{lang:'Croatian',code:'hr'},{lang:'Czech',code:'cs'},{lang:'Danish',code:'da'},{lang:'Dutch',code:'nl'},{lang:'English',code:'en'},{lang:'Esperanto',code:'eo'},{lang:'Estonian',code:'et'},{lang:'Filipino',code:'tl'},{lang:'Finnish',code:'fi'},{lang:'French',code:'fr'},{lang:'Galician',code:'gl'},{lang:'Georgian',code:'ka'},{lang:'German',code:'de'},{lang:'Greek',code:'el'},{lang:'Gujarati',code:'gu'},{lang:'Haitian ',code:'ht'},{lang:'Hebrew',code:'iw'},{lang:'Hindi',code:'hi'},{lang:'Hungarian',code:'hu'},{lang:'Icelandic',code:'is'},{lang:'Indonesian',code:'id'},{lang:'Irish',code:'ga'},{lang:'Italian',code:'it'},{lang:'Japanese',code:'ja'},{lang:'Kannada',code:'kn'},{lang:'Korean',code:'ko'},{lang:'Latin',code:'la'},{lang:'Latvian',code:'lv'},{lang:'Lithuanian',code:'lt'},{lang:'Macedonian',code:'mk'},{lang:'Malay',code:'ms'},{lang:'Maltese',code:'mt'},{lang:'Norwegian',code:'no'},{lang:'Persian',code:'fa'},{lang:'Polish',code:'pl'},{lang:'Portuguese',code:'pt'},{lang:'Romanian',code:'ro'},{lang:'Russian',code:'ru'},{lang:'Serbian',code:'sr'},{lang:'Slovak',code:'sk'},{lang:'Slovenian',code:'sl'},{lang:'Spanish',code:'es'},{lang:'Swahili',code:'sw'},{lang:'Swedish',code:'sv'},{lang:'Tamil',code:'ta'},{lang:'Telugu',code:'te'},{lang:'Thai',code:'th'},{lang:'Turkish',code:'tr'},{lang:'Ukrainian',code:'uk'},{lang:'Urdu',code:'ur'},{lang:'Vietnamese',code:'vi'},{lang:'Welsh',code:'cy'},{lang:'Yiddish',code:'yi'}
					])
