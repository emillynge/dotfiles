...
function Calendar(element, options, eventSources) {
	...
	function renderView(inc) {
		if (elementVisible()) {
			...
			// moved up because wasn't displaying
			//header.updateTitle(currentView.title);
			var today = new Date();
	...
	function setSize(dateChanged) { // todo: dateChanged?
		...
		// jason patch: because basic day was too small and couldn't scroll
		if (currentView.name == "basicDay") {
			currentView.setHeight(800, dateChanged);
		} else {		
	...
	function updateEvent(event) { // update an existing event
		...
		for (i=0; i<len; i++) {
			...
			if (e._id == event._id && e != event) {
				...
				e.color = event.color;
				// jason patch: originally mispelt from backgroudColor to backgroundColor 
...
function parseDate(s, ignoreTimezone) { // ignoreTimezone defaults to true
	...
	if (typeof s == 'string') {
		if (/^\d+(\.\d+)?$/.test(s)) { // a UNIX timestamp
...
function segCmp(a, b) {
	...
	// jason edit
	var retVal = a.event.start - b.event.start;	
	if (retVal == 0) {
		if (a.event.title && b.event.title) {
			if (a.event.title.toLowerCase() < b.event.title.toLowerCase()) {
				return -1;
			} else if (a.event.title.toLowerCase() > b.event.title.toLowerCase()) {
				return +1; 
			} else {
				return 0;
			}
...
function getSkinCss(event, opt, orientation) {
	var source = event.source || {};
	...
	// jason change: all day event should have the background set to their color and timed events should have their text to their color
	if (event.allDay || orientation == "vertical" || (event.start && event.end && !event.start.isSameDay(event.end))) {
		if (backgroundColor) {
			statements.push('background-color:' + backgroundColor);
		}
		if (borderColor) {
			statements.push('border-color:' + darkenColor(borderColor));
		}
		if (textColor) {
			statements.push('color:' + textColor);
		}
	} else {
		statements.push('border-color:transparent');
		statements.push('background-color:transparent');
		statements.push('color:' + darkenColor(backgroundColor));
	}
	...
	function updateCells() {
		...
		for (i=0; i<colCnt; i++) {
			...
			if (+date < +today) {
				...
				bodyCell.removeClass('fc-past');
			}

			if (+date == +today) {
				//console.log(i + "_" + new Date(+date) + " ++today: " + new Date(+today));
...
function AgendaEventRenderer() {
	...
	function slotSegHtml(event, seg) {
		var html = "<";
		var url = event.url;
		var skinCss = getSkinCss(event, opt, 'vertical');
		var skinCssAttr = (skinCss ? " style='" + skinCss + "'" : '');
		var classes = ['fc-event', 'fc-event-skin', 'fc-event-vert'];
		
		if (event.start < clearTime(new Date())) {
...
function DayEventRenderer() {
	...
	function renderDaySegs(segs, modifiedEventId) {
		...
		for (rowI=0; rowI<rowCnt; rowI++) {
			...
			var tooManyItemsToFit = false;
			
			while (i<segCnt && (seg = segs[i]).row == rowI) {
				//console.log("seg: " + seg.event.title);
				//console.log("seg: " + seg.event.title + " " + seg.level);
				
				// loop through segs in a row
				top = arrayMax(colHeights.slice(seg.startCol, seg.endCol));
				seg.top = top;
				
				seg.element.addClass("row" + rowI + "col" + seg.startCol);
				
				// jason: set maximum events per cell
				var MAX_VISIBLE_EVENTS_PER_DAY = 4;
				//console.log(rowI + " " + seg.startCol)				
				if ((seg.level+1) > MAX_VISIBLE_EVENTS_PER_DAY) { // had 40 previously
					//console.log("hide" + seg.event.title)
					tooManyItemsToFit = true;
					//console.log(seg)
					//top = 0;
					//seg.outerHeight = 0;
					//seg.event.title = "";
					//console.log(seg.level);
					if (seg.level == MAX_VISIBLE_EVENTS_PER_DAY) {
						//console.log("in 2222")
						
						var $moreDiv = $("<div class='moreLink' style='xxbackground:" + seg.event.color + "'>[more]</div>")
						$moreDiv.data("state", "more");
						
						$moreDiv.click({row:rowI, col:seg.startCol}, function(event) {
							var $hiddenEvents = $(".hiddenEvents.row" + event.data.row + "col" + event.data.col); 
							
							if ($(this).data("state") == "more") {
								$hiddenEvents.css({visibility:"visible", zIndex:20});
								$(this)
									.text("[less]")
									.data("state", "less")
								;
							} else {
								$hiddenEvents.css("visibility", "hidden");
								$(this)
									.text("[more]")								
									.data("state", "more")
								;
						...
						$(".row" + rowI + "col" + seg.startCol).eq(MAX_VISIBLE_EVENTS_PER_DAY-1).append($moreDiv);
					}						
					seg.element.addClass("hiddenEvents");
				}
			...
			if (t.name == "month" && tooManyItemsToFit) {
				var newHeight;
	...
	function daySegHTML(segs) { // also sets seg.left and seg.outerWidth
		...
		for (i=0; i<segCnt; i++) {
			...
			if (event.isSnoozer) {
				classes.push('fcSnoozerEvent');
			}
			if (event.isDeclined) {
				classes.push('fcDeclinedEvent');
			}
