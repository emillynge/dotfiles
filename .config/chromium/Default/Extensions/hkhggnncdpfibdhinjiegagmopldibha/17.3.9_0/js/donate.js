...
function initPaymentProcessor(price) {
	if (donateButtonClicked == "paypal") {
		...
		var $coinbaseWrapper = $("<div id='coinbaseWrapper' style='" + borderRadius + "-webkit-transition:top 800ms ease-in-out;left: 32%;top: 182px;text-align:center;position:fixed;background:white;width: 500px; height: 160px;box-shadow:2px 2px 193px rgba(0,0,0,1);'><img id='loadingCoinbase' src='/images/ajaxLoader.gif' style='height:20px;margin-top:56px'/></span><img id='closeCoinbase' src='images/closeBig.png' style='cursor:pointer;top: -16px;right: -16px;position: absolute;'/></div>");
...
$(document).ready(function() {
	...
	$(".alreadyDonated").click(function() {
		...
		if (bg.email) {
			$("#verifying").show();
			bg.Controller.verifyPayment(bg.itemID, bg.email, function(response) {
				$("#verifying").hide();
	...
	$("#multipleUserLicense").click(function(e) {
		$('#donateAmountDiv').hide();
		if (bg.email) {
			$("#choosePaymentWrapper").slideUp();
			$("#licenseDomain").text("@" + bg.email.split("@")[1]);
			// invalid domains					
			if (/@yahoo\.|@gmail\.|@mail\.|@comcast\.|@googlemail\.|@hotmail\./.test(bg.email)) {
